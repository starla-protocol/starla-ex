...
runner_path="$protocol_dir/scripts/run-core-http-claim.py"
port="${STARLA_EX_HTTP_PORT:-4747}"
base_url="http://127.0.0.1:${port}"
implementation_version="$(git rev-parse --short HEAD)"
output_path="${STARLA_EX_CLAIM_OUTPUT:-/tmp/starla-ex-core-http-report.md}"
...
STARLA_EX_HTTP_PORT="$port" mix run --no-halt >/tmp/starla-ex-core-http-claim.log 2>&1 &
server_pid="$!"
